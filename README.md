...
给 AI 编程助手的双层护栏插件：**spec 同步**减少跨 session 的目标漂移——你拍板的目标、边界、验收写回 spec（真源），下一轮不拿旧目标开工；**风险触发纪律**减少关键工程步骤被跳过——需求没对齐先对齐、可测的行为先写测试、修 bug 先找根因、说「做完了」先拿证据。8 个 skill + 2 个 hook，装一次全局生效，Claude Code 和 Codex 都能用。
