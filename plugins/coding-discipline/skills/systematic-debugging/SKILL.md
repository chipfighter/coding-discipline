...
description: 碰到任何 bug、测试失败、非预期行为时用，在提出任何修法之前。先复现、反向追到根因、在根上修一处、补回归测试——禁止瞎试。
---

铁律：**没定位到根因之前，不提任何修法。** 改症状不算修，是失败。
...
- 多组件系统（CI→构建→签名，API→服务→库）：在每个边界打点记"进什么/出什么"，跑一次看在**哪一层**断的，再钻那一层。别凭感觉猜是哪层。
...
## 禁瞎试（这些念头一冒就停，回第 1 步）
"先快修回头再查"、"改改 X 看行不行"、一把塞多个改动一起跑、"大概是 X 直接修"、还没追数据流就开方案、已经试了 2 次还想"再试一次"。
