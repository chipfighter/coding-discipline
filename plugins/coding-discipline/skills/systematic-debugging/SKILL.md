...
description: 碰到根因不明的 bug、测试失败或非预期行为时用，在提出永久修复之前——症状离根因有距离、修过一次没修好、或问题跨组件，必须走。先复现、反向追到根因、在根上修一处、补回归测试；紧急止血必须标明是可回退的 mitigation。不触发于：报错已直指原因的明显错误（语法/拼写/漏 import），直接修。
---
